...
# Initial Model Fit
model <-  'F1 =~ q1 + q3 + q6 + q7 + q13 + q16 + q18 + q19 + q21 +q39
           F2 =~ q33 + q36 + q38 + q41 + q42
           F3 =~ q2 + q9 + q10 + q23 + q27 + q29 + q30
...
items %>% 
  ...
  mutate(
    item = 
      case_when(
        var_label %in% c("q42", "q35", "q34", "q41") ~ paste0(item, "*"),
        TRUE ~ item
      ),
    Factor = c(rep("Factor 1", 8), rep("Factor 2", 7))
  ) %>% 
  gt() %>% 
  tab_header(
...
## Participants
```{r}
# df <- read.csv("data/clean/20231020_hr-scale-confirmatory-data2.csv")
# 
# 
# demos <- df %>% 
#   select(age, gid1, race, ed, sud_hx)
...
#               ) %>% 
#   as_gt() %>% 
#   tab_header(title = "Table X: Study 3 Participants")
```
